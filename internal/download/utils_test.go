...
func TestParsePath(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			gotImage, gotPath, err := ParsePath(tt.path)
			if tt.wantErr {
				if err == nil {
					t.Fatalf("ParsePath() expected error")
				}
				return
			}
			if err != nil {
			...
			if gotImage != tt.wantImage || gotPath != tt.wantPath {
				t.Fatalf("ParsePath() = (%q, %q), want (%q, %q)",
...
func TestHeaderFilename(t *testing.T) {
	...
	for _, tt := range tests {
		if got := HeaderFilename(tt.name); got != tt.want {
			t.Fatalf("HeaderFilename(%q) = %q, want %q", tt.name, got, tt.want)
		}
	}
}
